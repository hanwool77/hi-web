...
export const recommendService = {
  ...
  getPersonalizedStores: async (requestData) => {
    const response = await recommendApi.post('/api/recommend/stores', requestData);
    return response.data;
  },

  // 주변 매장 추천
  getNearbyStores: async (latitude, longitude, radius = 5000, category = null, page = 0, size = 20) => {
    const response = await recommendApi.get('/api/recommend/stores/nearby', {
  ...
  getPopularStores: async (category = null, page = 0, size = 20) => {
    const response = await recommendApi.get('/api/recommend/stores/popular', {
